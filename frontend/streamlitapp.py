...
# Configuration
# IMPORTANT: If deploying backend separately, change this URL
# For local development:
API_BASE_URL = "http://localhost:8000/api"
# For deployed backend (e.g., Render):
# API_BASE_URL = "https://your-fastapi-service.onrender.com/api"
                ...
                with st.spinner("Analyzing players and optimizing team..."):
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            st.error(f"Failed to generate team prediction: {response.status_code} - {response.text}")
                    except Exception as e:
                        st.error(f"Connection error to backend: {str(e)}. Ensure your FastAPI backend is running.")
                        # Display sample data for demo if connection fails
                        display_sample_team()
                ...
                with st.spinner("Getting strategic advice..."):
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            st.error(f"Could not get strategic advice: {response.status_code} - {response.text}")
                    except Exception as e:
                        st.warning(f"Strategic advice service unavailable: {str(e)}. Make sure EURI_API_KEY is set and backend is accessible.")
            ...
            if st.button("Get Chip Recommendations"):
                with st.spinner("Analyzing fixtures and generating recommendations..."):
                    try:
                        ...
                        if response.status_code == 200:
                            display_chip_recommendations_from_api(gameweek, available_chips, response.json()) # New function for API data
                        else:
                            st.error(f"Could not get chip recommendations: {response.status_code} - {response.text}")
                    except Exception as e:
                        st.warning(f"Chip recommendation service unavailable: {str(e)}. Displaying sample data.")
                        display_sample_chip_recommendations(gameweek, available_chips)  # Fallback to sample data
    ...
    for position in ['GKP', 'DEF', 'MID', 'FWD']:
        ...
        if not pos_players.empty:
            ...
            for _, player in pos_players.iterrows():
                # Corrected logic for captain_emoji
                captain_emoji = ""
                if player['id'] == team_data['captain']:
                    captain_emoji = "🔥 (C)"
                elif player['id'] == team_data['vice_captain']:
                    captain_emoji = "🅲 (VC)"
...
def display_chip_recommendations_from_api(gameweek, available_chips, recommendation_data):
    """Display chip strategy recommendations from API response"""
    # Confidence color coding
    confidence_color = ""
    if recommendation_data["confidence"] >= 0.8:
        confidence_color = "🟢"
    elif recommendation_data["confidence"] >= 0.6:
        confidence_color = "🟡"
    else:
        confidence_color = "🔴"
    ...
    with st.container():
        ...
        with col1:
            st.subheader(f"{recommendation_data['chip']} {confidence_color}")
            st.write(recommendation_data["reasoning"])
        with col2:
            st.metric("Recommended GW", recommendation_data["recommended_gameweek"])
            st.metric("Confidence", f"{recommendation_data['confidence']:.0%}")
        with col3:
            st.metric("Expected Benefit", f"+{recommendation_data['expected_benefit']:.1f} pts")
        st.divider()

def display_sample_chip_recommendations(gameweek, available_chips):
    """Display sample chip strategy recommendations for demo purposes"""
    ...
    st.info("Displaying sample chip recommendations as the service is unavailable.")
